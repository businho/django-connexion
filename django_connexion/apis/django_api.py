...
class DjangoApi:
    ...
    def __init__(self, specification: Union[str, Path, Dict], *args,
                 name='django_connexion', **kwargs):
        self.name = name
                ...
                with open(spec_path, 'r', encoding='utf-8') as f:
                    if spec_path.suffix.lower() in ['.yaml', '.yml']:
                        if HAS_YAML:
                            ...
                            raise ImportError("PyYAML is required to load YAML specifications")
                    else:
    ...
    def _get_openapi_json(self, request):
        ...
        try:
            ...
            return JsonResponse({"error": "Could not generate OpenAPI spec"}, status=500)
    ...
    def _add_operation(self, method: str, path: str, operation: Dict[str, Any]):
        ...
        try:
            ...
            view_func = self._create_view_function(operation_id, method, path, operation)

            # Create Django URL pattern
            endpoint_name = django_utils.djangofy_endpoint(operation_id, None)
            url_pattern = django_path(django_path_str.lstrip('/'), view_func, name=endpoint_name)

            self._url_patterns.append(url_pattern)
            logger.debug(f"Added URL pattern: {method} {django_path_str} -> {operation_id}")

        except Exception as e:
            logger.error(f"Error adding operation {method} {path}: {e}")

    def _get_path_parameter_types(self, operation: Dict[str, Any],
    ...
    def _create_view_function(self, operation_id: str, method: str, path: str,
        ...
        def view_func(request: HttpRequest, **kwargs):
            try:
                ...
                if method.upper() in ['GET', 'DELETE', 'HEAD', 'OPTIONS']:
                    ...
                    # For methods with potential body, pass request and all params as kwargs
                    result = handler_func(request, **all_params)
    ...
    def _process_query_params(self, request: HttpRequest,
    ...
    def _convert_path_params(self, django_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Convert parameter names from Django format (with underscores) to original format
        converted = {}
        for key, value in django_params.items():
            # Replace underscores with hyphens if needed (Django converts hyphens to underscores)
            converted[key] = value
        return converted
    ...
    def get_request(cls, request, *args, **params):
        """
        This method converts Django request to a ConnexionRequest for backward compatibility.
        ...
        try:
            ...
            connexion_request = ConnexionRequest(
                ...
                body=body,
                json_getter=lambda: (request.content_type == 'application/json' and
            ...
            logger.warning("ConnexionRequest not available, "
                           "compatibility method disabled")
            return None
    ...
    @classmethod
    def _build_response(cls, data, mimetype, content_type=None, status_code=None,
                        headers=None, extra_context=None):
        """
        ...
        if isinstance(data, (dict, list)):
            ...
            response = HttpResponse(data, content_type=content_type or mimetype,
